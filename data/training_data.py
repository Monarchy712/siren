...
coefficients that turn the 7 features into a sane, explainable risk_score.

Each sample is a 7-feature vector in engine.features.FEATURE_NAMES order:
...
generalize a little rather than memorizing three points.
"""
...
def build_dataset(seed: int = 7) -> tuple[list[list[float]], list[int]]:
    ...
    for _ in range(N_PER_ARCHETYPE):
        ...
        add([_u(rng, 0.0, 0.10), _u(rng, 0.0, 0.25), 0.0,
             _u(rng, 0.5, 1.0), _u(rng, 0.4, 1.0), 0.0, 0.0], 0)
        ...
        add([_u(rng, 0.0, 0.12), _u(rng, 0.0, 0.25), 0.0,
             _u(rng, 0.0, 0.08), _u(rng, 0.0, 0.02), 0.0, 0.0], 0)
        ...
        add([_u(rng, 0.55, 0.90), _u(rng, 0.0, 0.30), 0.0,
             _u(rng, 0.0, 0.10), _u(rng, 0.0, 0.03), 0.0, 0.0], 1)
